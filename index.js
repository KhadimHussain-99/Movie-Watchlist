...
function handleAddMovie(movieId) {
    // Check if the movie with the given ID is already in the addToLocalStorage array
    const movieExists = addToLocalStorage.some(movie => movie.imdbID === movieId)
  
    if (!movieExists) {
        const movie = searchedMovies.find(movie => movie.imdbID === movieId)
        if (movie) {
...
searchBtn.addEventListener('click', ()=> {
    ...
    fetch(`https://www.omdbapi.com/?apikey=a000a45b&s=${search.value}`)
        .then(res => res.json())
        .then(data => {
            data.Search.map( movie => {
                fetch(`https://www.omdbapi.com/?apikey=a000a45b&t=${movie.Title}`)
                    .then(res => res.json())
                    .then(data => {
